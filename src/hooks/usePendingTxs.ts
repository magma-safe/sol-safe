...
import { selectSolTxBySafe } from '@/store/solVaultTxnSlice'
import { SolVaultTxnType, SolTxnStatusType } from '@/utils/solvaulthelper'
...
export const usePendingTxs = (): Array<SolVaultTxnType> => {
  const createKey = useSafeCreateKey()
  const { publicKey } = useWallet()
  return useAppSelector((state) => selectSolTxBySafe(state, createKey, publicKey ? publicKey.toBase58() : '', true))
}

export const useNonPendingTxs = (): Array<SolVaultTxnType> => {
  const createKey = useSafeCreateKey()
  const { publicKey } = useWallet()
  return useAppSelector((state) => selectSolTxBySafe(state, createKey, publicKey ? publicKey.toBase58() : '', false))
}
export const useHasPendingTxs = (): boolean => {
  // const pendingIds = usePendingTxIds()
  const txns = usePendingTxs()
