...
const createInitialVaultTxn = (
  safeTx: TransactionInstruction[],
  extraInfo: {
    blockHash: string
  ...
  const [vaultPda] = multisig.getVaultPda({
    multisigPda: extraInfo.multisigPda,
    index: 0,
  })

  const transferMessage = new TransactionMessage({
    payerKey: vaultPda,
    recentBlockhash: extraInfo.blockHash,
    instructions: [safeTx[safeTx.length - 1]],
  })

  return multisig.instructions.vaultTransactionCreate({
    multisigPda: extraInfo.multisigPda,
    transactionIndex: extraInfo.txnIndex,
    creator: extraInfo.userAddress,
    vaultIndex: 0,
    ephemeralSigners: 0,
    transactionMessage: transferMessage,
  })

  // const vaultTxnCreateInstruction = multisig.transactions.vaultTransactionCreate({
  //   blockhash: extraInfo.blockHash,
  //   feePayer: extraInfo.userAddress,
  //   multisigPda: extraInfo.multisigPda,
  //   transactionIndex: extraInfo.txnIndex,
  //   creator: extraInfo.userAddress,
  //   vaultIndex: 0,
  ...
  // return vaultTxnCreateInstruction
}

const createVaultTxnProposal = (extraInfo: {
  blockHash: string
  multisigPda: PublicKey
  txnIndex: bigint
  userAddress: PublicKey
...
const approveVaultTxnProposal = (extraInfo: {
  blockHash: string
  multisigPda: PublicKey
  txnIndex: bigint
  userAddress: PublicKey
...
const createVaultTxn = async (
  ...
  const txnIndex = BigInt(currentTransactionIndex + 1)
  const blockHash = (await connection.getLatestBlockhash()).blockhash

  const instructions: TransactionInstruction[] = []
  if (safeTx.length > 1) {
    instructions.push(safeTx[0])
  }

  const vaultTxnInstruction = createInitialVaultTxn(safeTx, {
    blockHash: blockHash,
  ...
  const proposalTxnInstruction = createVaultTxnProposal({
    blockHash,
    multisigPda,
    txnIndex,
    userAddress,
  ...
  const voteTxnInstruction = approveVaultTxnProposal({
    blockHash,
    multisigPda,
    txnIndex,
    userAddress,
  ...
  instructions.push(vaultTxnInstruction)

  instructions.push(proposalTxnInstruction)

  instructions.push(voteTxnInstruction)
 ...
 * Sign a transaction
 */
export const dispatchSolTxSigning = async (
  safeTxs: TransactionInstruction[],
  connection: Connection,
  ...
  try {
    signedTxId = await createVaultTxn(safeTxs, connection, vaultAddress, userAddress, sendTransaction)
  } catch (error) {
    txDispatch(TxEvent.SIGN_FAILED, {
      txId: '',
      error: asError(error),
  ...
  txDispatch(TxEvent.SIGNED, { txId: signedTxId })
