...
export const useSolTxActions = (): SolTxActions => {
  const { safe } = useSafeInfo()
  const onboard = useOnboard()
  const wallet = useWallet()
  const [addTxToBatch] = useUpdateBatch()
  ...
  return useMemo<SolTxActions>(() => {
    ...
    const proposeTx: SolTxActions['proposeTx'] = async (safeTx, txId, origin) => {
      assertTx(safeTx)
      return _propose(wallet?.address || safe.owners[0].value, safeTx, txId, origin)
    }

    const addToBatch: SolTxActions['addToBatch'] = async (safeTx, origin) => {
      assertTx(safeTx)
      assertWallet(wallet)
    ...
    const signTx: SolTxActions['signTx'] = async (
      safeTx: TransactionInstruction[],
      connection: Connection,
      vaultCreateKey: string,
      userAddress: PublicKey,
      ...
      const signedTxnHash = await dispatchSolTxSigning(safeTx, connection, vaultCreateKey, userAddress, sendTransaction)
      return signedTxnHash
    }

    const signDelegateTx: SolTxActions['signDelegateTx'] = async (safeTx) => {
      assertTx(safeTx)
      assertWallet(wallet)
      assertOnboard(onboard)
    ...
    const executeTx: SolTxActions['executeTx'] = async (txOptions, safeTx, txId, origin, isRelayed) => {
      assertTx(safeTx)
      assertWallet(wallet)
      assertOnboard(onboard)
      assertChainInfo(chain)
