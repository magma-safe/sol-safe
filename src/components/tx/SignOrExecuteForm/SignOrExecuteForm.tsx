...
import SignSolForm from './SignSolForm'
...
export const SignOrExecuteForm = ({
  ...
  safeSolTxInstructions: ReturnType<typeof useSafeSolTxInstructions>
  ...
  const isBatchable = props.isBatchable
      ...
      <TxCard>
        ...
        {!isCounterfactualSafe && !willExecute && !willExecuteThroughRole && !isDelegate && (
          <SignSolForm
            {...props}
            // safeTx={safeTx}
            safeSolTxInstructions={safeSolTxInstructions}
...
const useSafeSolTxInstructions = () => useContext(SafeTxContext).safeSolTxInstructions

export default madProps(SignOrExecuteForm, {
  chainId: useChainId,
  safeTx: useSafeTx,
  safeTxError: useSafeTxError,
  safeSolTxInstructions: useSafeSolTxInstructions
})
