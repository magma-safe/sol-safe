...
export const useGlobalImportSolJsonParser = (jsonData: string | undefined): SolData => {
  return useMemo(() => {
    const data: SolData = {
      safes: [],
    }
    ...
    if (!parsedFile || !Array.isArray(parsedFile) || Object.keys(parsedFile).length === 0) {
      data.error = ImportErrors.NO_IMPORT_DATA_FOUND
      return data
    }

    const safes: SolSafeItem[] = []

    for (const item of parsedFile) {
      safes.push({
        safeAddress: item.safeAddress,
        linkedAddress: item.linkedAddress,
