...
export const TxSigners = ({ txDetails, txSummary }: TxSignersProps): ReactElement | null => {
  ...
  const confirmationsCount = txn.approved.length
  const canExecute = wallet?.address ? isExecutable(txSummary, wallet.address, safe) : false
  const confirmationsNeeded = safe.threshold - confirmationsCount
      ...
      <List data-testid="transaction-actions-list" className={css.signers}>
        ...
        <ListItem>
          ...
          <ListItemText data-testid="confirmation-action" primaryTypographyProps={{ fontWeight: 700 }}>
            Confirmations{' '}
            <Box className={css.confirmationsTotal}>({`${confirmationsCount} of ${safe.threshold}`})</Box>
          </ListItemText>
        </ListItem>
        {!hideConfirmations &&
          txn.approved.map((item) => (
            <ListItem key={item} sx={{ py: 0 }}>
              <StyledListItemIcon $state={StepState.CONFIRMED}>
                <Dot />
              </StyledListItemIcon>
              <ListItemText data-testid="signer">
                <EthHashInfo address={item} hasExplorer showCopyButton />
              </ListItemText>
            </ListItem>
          ))}
